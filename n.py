...
def init_session_state():
    session_defaults = {
        ...
        'user_type': None,
        'video_questions': [
            "Tell us about your name, gender, and ethnicity",
            ...
            "Where are you located and are you open to travel?",
        ...
        'profiler': None,
        'validation_metrics': {
            'last_extraction_accuracy': None,
            'last_personality_correlation': None,
...
def register_user(username: str, password: str, user_type: str, profile_data: Dict) -> Tuple[bool, str]:
...
class ApprenticeProfiler:
            ...
            try:
                with st.spinner("Loading AI models..."):
                    try:
                        ...
                        st.error(f"Whisper model error: {str(e)}")
                    ...
                    try:
                        ...
                        st.error(f"Sentiment model error: {str(e)}")
            except Exception as e:
        ...
        try:
            if 'whisper' in self.models:
                ...
                with st.spinner("Transcribing audio with AI model..."):
                    ...
                    end_time = time.time()
                    st.session_state.validation_metrics['last_processing_time'] = end_time - start_time
                    return result['text']
            else:
    ...
    def extract_candidate_info(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Name extraction using NER if available
            ...
            # Skill extraction with validation
            skill_keywords = [
            ...
            info['skills'] = list(set(validated_skills))
            ...
            if education_sentences:
                info['education'] = '. '.join(education_sentences[:3])
            
            # Experience extraction
            experience_keywords = ['experience', 'worked', 'position', 'role', 'job', 'company', 'years', 'responsibilities']
            ...
            if experience_sentences:
                info['experience'] = '. '.join(experience_sentences[:4])
            
            # Calculate extraction accuracy
            extracted_fields = [key for key, value in info.items() if value]
            accuracy = len(extracted_fields) / len(info) * 100
            st.session_state.validation_metrics['last_extraction_accuracy'] = accuracy
    ...
    def analyze_personality(self, text: str) -> Dict[str, float]:
        try:
            ...
            trait_keywords = {
                'openness': ['creative', 'innovative', 'curious', 'imaginative', 'artistic', 'original', 'inventive', 'experimental', 'visionary'],
                'conscientiousness': ['organized', 'responsible', 'detail', 'planning', 'systematic', 'thorough', 'careful', 'disciplined', 'reliable', 'efficient'],
                'extraversion': ['outgoing', 'social', 'energetic', 'talkative', 'assertive', 'enthusiastic', 'active', 'gregarious', 'sociable'],
                'agreeableness': ['cooperative', 'helpful', 'kind', 'supportive', 'friendly', 'empathetic', 'considerate', 'compassionate', 'trusting'],
                'neuroticism': ['anxious', 'worried', 'stressed', 'emotional', 'sensitive', 'nervous', 'tense', 'moody', 'insecure']
            }
            ...
            for trait, words in trait_keywords.items():
                trait_count = sum(1 for word in words if re.search(rf'\b{word}\b', text_lower))
                trait_density = trait_count / word_count if word_count > 0 else 0
                personality[trait] = min(1.0, max(0.0, 0.5 + (trait_density * 2)))
            
            # Validation - calculate correlation with psychometric test if available
            if 'psychometric_scores' in st.session_state.get('current_candidate', {}):
                psych_scores = st.session_state.current_candidate['psychometric_scores']
                traits = ['openness', 'conscientiousness', 'extraversion', 'agreeableness', 'neuroticism']
                correlations = []
                for trait in traits:
                    if trait in psych_scores and trait in personality:
                        # Normalize psych score to 0-1 range (original is 1-7)
                        norm_score = (psych_scores[trait] - 1) / 6
                        correlations.append(abs(personality[trait] - norm_score))
                avg_correlation = 1 - (sum(correlations) / len(correlations)) if correlations else 0
                st.session_state.validation_metrics['last_personality_correlation'] = avg_correlation
    ...
    def generate_insights(self, candidate_data: Dict, transcript: str, 
        ...
        try:
            if groq_api_key and GROQ_AVAILABLE:
                ...
                prompt = f"""
**Objective:** Generate highly accurate and professional insights about the candidate based on provided information.
...
- **Skills:** {', '.join(candidate_data.get('skills', ['None identified']))}
...
- **Openness:** {personality.get('openness', 0.5):.2f} (Creative, imaginative, open to new experiences)
- **Conscientiousness:** {personality.get('conscientiousness', 0.5):.2f} (Organized, responsible, detail-oriented)
- **Extraversion:** {personality.get('extraversion', 0.5):.2f} (Outgoing, energetic, socially confident)
- **Agreeableness:** {personality.get('agreeableness', 0.5):.2f} (Cooperative, empathetic, team-oriented)
- **Neuroticism:** {personality.get('neuroticism', 0.5):.2f} (Emotionally sensitive, reactive to stress)

**Transcript Excerpt:**
{transcript[:1000]}{'...' if len(transcript) > 1000 else ''}

**Analysis Guidelines:**
1. Provide factual, evidence-based insights
2. Quantify observations where possible
3. Cross-reference information for consistency
4. Highlight both strengths and development areas
5. Provide specific, actionable recommendations

**Output Structure:**
### Professional Summary
Concise overview of candidate's profile
...
"""
    ...
    def generate_rule_based_insights(self, candidate_data: Dict, personality: Dict) -> str:
        ...
        if candidate_data.get('skills'):
            ...
            insights.append(f"- Demonstrates {skills_count} validated technical and soft skills")
        
        if candidate_data.get('education'):
            insights.append(f"- Educational background: {candidate_data['education'][:100]}{'...' if len(candidate_data['education']) > 100 else ''}")
            
        if candidate_data.get('experience'):
            insights.append(f"- Professional experience: {candidate_data['experience'][:100]}{'...' if len(candidate_data['experience']) > 100 else ''}")
        ...
        if skills:
            technical_skills = [s for s in skills if any(tech in s.lower() for tech in ['python', 'java', 'sql', 'react', 'aws', 'docker', 'machine'])]
            soft_skills = [s for s in skills if any(soft in s.lower() for soft in ['leadership', 'communication', 'teamwork', 'problem', 'analytical'])]
            
            if technical_skills:
                insights.append(f"- **Technical Proficiency**: {', '.join(technical_skills[:5])}")
            if soft_skills:
                insights.append(f"- **Interpersonal Skills**: {', '.join(soft_skills[:5])}")
            if len(skills) > 10:
                insights.append(f"- **Skill Diversity**: Demonstrates broad skill set with {len(skills)} validated competencies")
        else:
        ...
        for trait, score in sorted_traits:
            level = "High" if score > 0.7 else "Moderate" if score > 0.5 else "Low"
...
def render_login_page():
    ...
    with tab2:
        ...
        if user_type == "apprentice":
            ...
            profile_data = {"name": name, "location": location, "email": email, "availability": True}
        elif user_type == "company":
        ...
        if st.button("Register"):
            if reg_username and reg_password:
                success, message = register_user(reg_username, reg_password, user_type, profile_data)
                if success:
...
def render_apprentice_profile(user_id: str, profile: Dict):
    ...
    with col2:
        ...
        name = st.text_input("Name*", value=profile.get('name', ''), help="Required field")
        location = st.text_input("Location*", value=profile.get('location', ''), help="Required field")
        ...
        if st.button("Update Profile"):
            if not name or not location:
                st.error("Name and location are required fields")
                return
...
def render_opportunities():
    ...
    opportunities = [
        {"title": "Software Developer Apprenticeship", "company": "Tech Corp", "location": "London", "type": "Technology", "match_score": 92},
        {"title": "Digital Marketing Apprentice", "company": "Marketing Plus", "location": "Manchester", "type": "Marketing", "match_score": 87},
        {"title": "Data Analysis Trainee", "company": "Data Solutions", "location": "Birmingham", "type": "Data Science", "match_score": 95},
        {"title": "Mechanical Engineering Apprentice", "company": "Engineering Ltd", "location": "Glasgow", "type": "Engineering", "match_score": 78}
    ]
    ...
    with col3:
        min_match = st.slider("Minimum Match Score", 0, 100, 80)
    
    for opp in opportunities:
        if (location_filter == "All" or opp["location"] == location_filter) and \
           (type_filter == "All" or opp["type"] == type_filter) and \
           opp["match_score"] >= min_match:
            
            with st.expander(f"{opp['title']} - {opp['company']}  <span class='accuracy-badge'>{opp['match_score']}% match</span>"):
...
def render_profile_analyzer():
    ...
    with col2:
        ...
        if any(metrics.values()):
            ...
            with col_metrics[0]:
                if metrics['last_extraction_accuracy'] is not None:
                    st.metric("Extraction Accuracy", f"{metrics['last_extraction_accuracy']:.1f}%")
            with col_metrics[1]:
                if metrics['last_personality_correlation'] is not None:
                    st.metric("Personality Correlation", f"{metrics['last_personality_correlation']:.2f}")
            with col_metrics[2]:
                if metrics['last_processing_time'] is not None:
                    st.metric("Processing Time", f"{metrics['last_processing_time']:.2f}s")
        
        if transcript and len(transcript.strip()) > 20:
            user_id = st.session_state.users[st.session_state.current_user]['id']
            st.session_state.current_candidate = st.session_state.apprentices.get(user_id, {})
...
def render_company_profile(user_id: str, profile: Dict):
    ...
    with col1:
        company_name = st.text_input("Company Name*", value=profile.get('company_name', ''), help="Required field")
        industry = st.text_input("Industry*", value=profile.get('industry', ''), help="Required field")
        location = st.text_input("Location*", value=profile.get('location', ''), help="Required field")
    ...
    if st.button("Update Profile"):
        if not company_name or not industry or not location:
            st.error("Company name, industry, and location are required fields")
            return
...
def render_candidate_search(profile: Dict):
    ...
    if st.button("Search Candidates"):
        ...
        for candidate in candidates:
            ...
            with st.expander(f"{candidate['name']} - {candidate['location']}  <span class='accuracy-badge'>{match_score}% match</span>"):
...
def render_company_analytics():
    ...
    st.subheader("Match Accuracy by Role")
    roles = ['Developer', 'Analyst', 'Marketer', 'Engineer']
    accuracy = [92, 88, 85, 78]
    fig = px.bar(x=roles, y=accuracy, title="Match Accuracy (%)")
    st.plotly_chart(fig, use_container_width=True)
...
def render_tp_profile(user_id: str, profile: Dict):
    ...
    with col1:
        provider_name = st.text_input("Provider Name*", value=profile.get('provider_name', ''), help="Required field")
        specialization = st.text_input("Specialization*", value=profile.get('specialization', ''), help="Required field")
    ...
    if st.button("Update Profile"):
        if not provider_name or not specialization:
            st.error("Provider name and specialization are required fields")
            return
...
def render_apprentice_management():
    ...
    apprentices = [
        {"name": "John Doe", "program": "Digital Marketing", "progress": 75, "status": "Active", "accuracy": 92},
        {"name": "Jane Smith", "program": "Software Development", "progress": 60, "status": "Active", "accuracy": 88},
        {"name": "Mike Johnson", "program": "Data Analysis", "progress": 90, "status": "Near Completion", "accuracy": 95}
    ]
    
    for apprentice in apprentices:
        with st.expander(f"{apprentice['name']} - {apprentice['program']}  <span class='accuracy-badge'>{apprentice['accuracy']}% accuracy</span>", unsafe_allow_html=True):
...
def render_tp_collaboration():
    ...
    companies = [
        {"name": "Tech Corp", "industry": "Technology", "active_apprentices": 15, "success_rate": 92},
        {"name": "Marketing Plus", "industry": "Marketing", "active_apprentices": 8, "success_rate": 87},
        {"name": "Data Solutions", "industry": "Data Science", "active_apprentices": 12, "success_rate": 95}
    ]
    
    for company in companies:
        with st.expander(f"{company['name']} - {company['industry']}  <span class='accuracy-badge'>{company['success_rate']}% success</span>", unsafe_allow_html=True):
...
def render_tp_analytics():
    ...
    st.subheader("Placement Accuracy")
    program_data = {
        'Program': ['Digital Marketing', 'Software Development', 'Data Analysis', 'Engineering'],
        'Placement Rate': [92, 88, 95, 78],
        'Accuracy Score': [94, 90, 96, 82]
    }
...
def render_system_config():
    ...
    with col1:
        ...
        ai_accuracy = st.slider("AI Accuracy Threshold", 0, 100, 85)
        min_confidence = st.slider("Minimum Confidence Level", 0.0, 1.0, 0.7)
    ...
    with col2:
        ...
        log_accuracy = st.checkbox("Log Accuracy Metrics", value=True)
...
def render_admin_analytics():
    ...
    with col2:
        st.metric("Profile Accuracy", "92.7%", "+3.1%")
    with col3:
    ...
    with col2:
        ...
        accuracy = [94, 89, 92, 86]
        fig = px.bar(x=regions, y=accuracy, title="Accuracy by Region (%)")
        st.plotly_chart(fig, use_container_width=True)
...
def render_content_management():
    ...
    st.write("**AI Model Validation**")
    if st.button("Run Validation Tests"):
        st.info("Running validation tests...")
        time.sleep(2)
        st.session_state.validation_metrics = {
            'last_extraction_accuracy': 92.5,
            'last_personality_correlation': 0.87,
            'last_processing_time': 3.24
        }
        st.success("Validation tests completed successfully!")
...
def render_admin_reports():
    ...
    with col2:
        ...
        if st.button("Generate Report"):
            ...
            elif report_type == "Accuracy Report":
                data = {
                    'Module': ['Profile Extraction', 'Personality Analysis', 'Matching Algorithm'],
                    'Accuracy': [92.5, 87.3, 94.1],
                    'Improvement': [2.4, 3.1, 1.8]
                }
            elif report_type == "Matching Success Report":
                data = {
                    'Industry': ['Technology', 'Marketing', 'Engineering', 'Healthcare'],
                    'Matches': [156, 89, 134, 78],
                    'Success Rate': [0.92, 0.87, 0.89, 0.85]
                }
            else:
                data = {
                    'Component': ['Video Processing', 'PDF Extraction', 'AI Analysis'],
                    'Avg Time (s)': [3.2, 1.8, 4.5],
                    'Reliability': [98.7, 99.2, 97.8]
                }
